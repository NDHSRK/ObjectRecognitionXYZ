...
# This class is a subset of the FTC Java class VisionParameters.ImageParameters.
# In Java the fields come from an XML file; for Python we'll change image_source,
# which in Java is a file name or "webcam", to hold the actual full-size image.
'''
...
class ImageParameters:
    def __init__(self, p_image, p_resolution_width, p_resolution_height,
                 p_roi_x, p_roi_y, p_roi_width, p_roi_height):
        self.image = p_image
        self.resolution_width = p_resolution_width
        self.resolution_height = p_resolution_height
        self.roi_x = p_roi_x
        self.roi_y = p_roi_y
        self.roi_width = p_roi_width
        self.roi_height = p_roi_height
...
class SampleRecognition:
    ...
    # This tester runs a single use case in which the image
    # is read from a file. But when we get to the real-world
    # CalculateXYZ we will run multiple images from a webcam
    # through the recognition code. Not only that, the ROI
    # definition may change when we get images from the webcam
    # after the toy picker has changed position.
    ...
    # The final return value from perform_recognition is
    # a class object (SampleRecognitionReturn) that contains
    # a status and a list of class objects, each of which
    # contains the x and y coordinates and the FTC angle of
    # a recognized object.
    ...
    def perform_recognition(self, p_full_image, p_image_parameters):
        roi = ImageUtils.pre_process_image(p_full_image,
                                           p_image_parameters.roi_x, p_image_parameters.roi_y,
                                           p_image_parameters.roi_width, p_image_parameters.roi_height)
        ...
        filtered = ImageUtils.filter_contours(thresholded, p_image_parameters.roi_width, p_image_parameters.roi_height)
        if not filtered:
            print("No valid contours found")
            return self.SampleRecognitionReturn(self.SampleRecognitionReturn.RecognitionStatus.FAILURE, [])

        rectangles = self.enclose_and_filter_samples(filtered, p_image_parameters.roi_width, p_image_parameters.roi_height,
    ...
    # You can use width and height to check aspect ratio.
    @staticmethod
    def enclose_and_filter_samples(p_filtered_contours, image_width, image_height, min_area, max_area):
        filtered_rects = []
